...
function reference_semantics(): void {
    ...
    $vec = Vector{1, 2, 3};
    $cp_vec = $vec; // 2つのVectorは同じ参照を持つ
    $vec[0] = 10; // $vecに加えた変更は$cp_vecにも影響する
    var_dump($vec);
    var_dump($cp_vec);
    mod_vector($vec);
    var_dump($cp_vec); // 関数呼び出し後も参照に影響
}

function value_semantics(): void {
    echo "==== value semantics ====" . PHP_EOL;
    $arr = array(1, 2, 3);
    $cp_arr = $arr; // 2つの配列は同じ値を持つがコピーではない
    $arr[0] = 10; // $cp_arrは影響を受けない
    var_dump($arr);
    var_dump($cp_arr);
    mod_array($arr);
    var_dump($cp_arr); // 関数呼び出し後も$cp_arrは影響を受けない
}
